...
int analyse_beam(TString source, TString out, TString gen, Int_t ring, TString mod){
...
for (size_t j=0;j< nEvents;j++){
	T.GetEntry(j);
		...
		Double_t sepdiv=2*TMath::Pi()/7.0;
  		...
  		if (secphi<TMath::Pi()/28.0){sec=1;}           // closed 
                ...
                if (mod=="Quartz"){
                  ...
                  min_trans[5]=0; max_trans[5]=2000;
                  ...
                  min_closed[5]=0; max_closed[5]=2000;
		...
		std::map<TString, Bool_t> hit_pid;
                ...
                hit_pid["all"]=1;
                if(gen=="beam"){
			hit_pid["primary"]=hit.pid==11 && hit.vz<=-3800;
			hit_pid["electron"]=hit.pid==11 && hit.vz>-3800;
			hit_pid["positron"]=hit.pid==-11;
			hit_pid["photon"]= hit.pid==22; 
			hit_pid["other"]= hit.pid!=11 && hit.pid!=-11 && hit.pid!=22;
		}else{
			hit_pid["primary"]= hit.trid<=max_track;
                        hit_pid["electron"]=hit.pid==11 && hit.trid>max_track;
			hit_pid["positron"]=hit.pid==-11;
			hit_pid["photon"]=hit.pid==22;
			hit_pid["other"]=hit.pid!=11 && hit.pid!=-11 && hit.pid!=22;
