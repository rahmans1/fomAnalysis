...
int analyse(TString source, TString out, TString gen, Int_t ring, TString mod){
...
std::map<TString, TH1D*> h_fom_open;
std::map<TString, TH1D*> h_fom_trans;
std::map<TString, TH1D*> h_fom_closed;

std::map<TString, TH1D*> h_asy;
std::map<TString, TH1D*> h_asy_open;
std::map<TString, TH1D*> h_asy_trans;
std::map<TString, TH1D*> h_asy_closed;
...
h_fom["primary"]=new TH1D("pr_fom", Form("primary FOM, Ring = %d, Sector = all, Generator = %s, Part = %s", ring, gen.Data(), mod.Data()), 400, 0, 2000);
h_fom_open["primary"]=new TH1D("pr_fom_open", Form("primary FOM, Ring = %d, Sector = open, Generator = %s, Part = %s", ring, gen.Data(), mod.Data()), 400, 0, 2000);
h_fom_trans["primary"]=new TH1D("pr_fom_trans", Form("primary FOM, Ring = %d, Sector = trans, Generator = %s, Part = %s", ring, gen.Data(), mod.Data()), 400, 0, 2000);
h_fom_closed["primary"]=new TH1D("pr_fom_closed", Form("primary FOM, Ring = %d, Sector = closed, Generator = %s, Part = %s", ring, gen.Data(), mod.Data()), 400, 0, 2000);

if(gen=="moller"){
h_asy["primary"]=new TH1D("pasy", Form("primary asy, Ring = %d, Sector = all, Generator = %s, Part = %s", ring, gen.Data(), mod.Data()), 300, -60, 0);
h_asy_open["primary"]=new TH1D("pasy_open", Form("primary asy, Ring = %d, Sector = open, Generator = %s, Part = %s", ring, gen.Data(), mod.Data()), 300, -60, 0);
h_asy_trans["primary"]=new TH1D("pasy_trans", Form("primary asy, Ring = %d, Sector = trans, Generator = %s, Part = %s", ring, gen.Data(), mod.Data()), 300, -60, 0);
h_asy_closed["primary"]=new TH1D("pasy_closed", Form("primary asy, Ring = %d, Sector = closed, Generator = %s, Part = %s", ring, gen.Data(), mod.Data()), 300, -60, 0);
}else if(gen=="elastic"){
h_asy["primary"]=new TH1D("pasy", Form("primary asy, Ring = %d, Sector = all, Generator = %s, Part = %s", ring, gen.Data(), mod.Data()), 50, -150, 0);
h_asy_open["primary"]=new TH1D("pasy_open", Form("primary asy, Ring = %d, Sector = open, Generator = %s, Part = %s", ring, gen.Data(), mod.Data()), 50, -150, 0);
h_asy_trans["primary"]=new TH1D("pasy_trans", Form("primary asy, Ring = %d, Sector = trans, Generator = %s, Part = %s", ring, gen.Data(), mod.Data()), 50, -150, 0);
h_asy_closed["primary"]=new TH1D("pasy_closed", Form("primary asy, Ring = %d, Sector = closed, Generator = %s, Part = %s", ring, gen.Data(), mod.Data()), 50, -150, 0);
}else{
h_asy["primary"]=new TH1D("pasy", Form("primary asy, Ring = %d, Sector = all, Generator = %s, Part = %s", ring, gen.Data(), mod.Data()), 400, -2000, 0);
h_asy_open["primary"]=new TH1D("pasy_open", Form("primary asy, Ring = %d, Sector = open, Generator = %s, Part = %s", ring, gen.Data(), mod.Data()), 400, -2000, 0);
h_asy_trans["primary"]=new TH1D("pasy_trans", Form("primary asy, Ring = %d, Sector = trans, Generator = %s, Part = %s", ring, gen.Data(), mod.Data()), 400, -2000, 0);
h_asy_closed["primary"]=new TH1D("pasy_closed", Form("primary asy, Ring = %d, Sector = closed, Generator = %s, Part = %s", ring, gen.Data(), mod.Data()), 400, -2000, 0);
} 
		...
		Double_t sepdiv=2*TMath::Pi()/7.0;
  		...
  		if (secphi<TMath::Pi()/28.0){sec=1;}           // closed 
                ...
                if (mod=="Quartz"){
                  ...
                  min_open[5]=600; max_open[5]=1300;
                  ...
                  min_trans[5]=600; max_trans[5]=1300;
                  ...
                  min_closed[5]=600; max_closed[5]=1300;
			...
			{"other", hit.trid>max_track && hit.pid!=11 && hit.pid!=-11 && hit.pid!=22}
		  	...
		  	if (hit_radial && hit_pid[particle[i]]){                         
				 ...
				 h_closed[particle[i]]->Fill(hit.r, (fRate)*weight);
                           ...
                           h[particle[i]]->Fill(hit.r, (fRate)*weight);
                           h_fom[particle[i]]->Fill(hit.r, (fRate)*(fEvent->A)*(fEvent->A)*weight);
                           h_asy[particle[i]]->Fill(fEvent->A, fRate*weight);
                        }
		} 
