...
int analyse_rasterStudy(TString source, TString out, TString gen, Int_t ring, TString mod){
...
particle.push_back(Form("primary_%d", i*250));

h[particle[i]]=new TH1D(Form("pr_%s", particle[i].Data()), Form("%s, Ring = %d, Sector = all, Generator = %s, Part = %s",particle[i].Data(), ring, gen.Data(), mod.Data()), 400, 0, 2000);
h_open[particle[i]]=new TH1D(Form("pr_open_%s", particle[i].Data()), Form("%s, Ring = %d, Sector = open, Generator = %s, Part = %s",particle[i].Data(), ring, gen.Data(), mod.Data()), 400, 0, 2000);
h_trans[particle[i]]=new TH1D(Form("pr_trans_%s", particle[i].Data()), Form("%s, Ring = %d, Sector = trans, Generator = %s, Part = %s",particle[i].Data(), ring, gen.Data(), mod.Data()), 400, 0, 2000);
h_closed[particle[i]]=new TH1D(Form("pr_closed_%s", particle[i].Data()), Form("%s, Ring = %d, Sector = closed, Generator = %s, Part = %s",particle[i].Data(), ring, gen.Data(), mod.Data()), 400, 0, 2000);

h_fom[particle[i]]=new TH1D(Form("pr_fom_%s", particle[i].Data()), Form("%s FOM, Ring = %d, Sector = all, Generator = %s, Part = %s",particle[i].Data(), ring, gen.Data(), mod.Data()), 400, 0, 2000);
h_fom_open[particle[i]]=new TH1D(Form("pr_fom_open_%s", particle[i].Data()), Form("%s FOM, Ring = %d, Sector = open, Generator = %s, Part = %s",particle[i].Data(), ring, gen.Data(), mod.Data()), 400, 0, 2000);
h_fom_trans[particle[i]]=new TH1D(Form("pr_fom_trans_%s", particle[i].Data()), Form("%s FOM, Ring = %d, Sector = trans, Generator = %s, Part = %s",particle[i].Data(), ring, gen.Data(), mod.Data()), 400, 0, 2000);
h_fom_closed[particle[i]]=new TH1D(Form("pr_fom_closed_%s", particle[i].Data()), Form("%s FOM, Ring = %d, Sector = closed, Generator = %s, Part = %s",particle[i].Data(), ring, gen.Data(), mod.Data()), 400, 0, 2000);
}
...
for (size_t j=0;j< nEvents;j++){
	T.GetEntry(j);
		...
		std::map<TString, Bool_t> hit_pid;
                ...
                for(Int_t i=0; i<particle.size();i++){
		   hit_pid[particle[i]]= hit.trid<=max_track && abs( sqrt(hit.vx*hit.vx+hit.vy*hit.vy)/(hit.vz+19810)- i*0.25/(19810-4500))<=1e-6 ;
